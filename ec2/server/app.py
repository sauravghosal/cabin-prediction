...
def query_occupancy_by_date_range(table_name, start_date, end_date):
    try:
        ...
        return conn.execute(f"""
          SELECT * FROM `{table_name}` WHERE date BETWEEN '{start_date:%Y-%m-%d}' AND '{end_date:%Y-%m-%d}'
          """)
...
def home():
    response = requests.get('http://18.117.140.76/api',
                            params={'start_date': '10-10-2019', 'end_date': '11-10-2019', 'cabin': 'Smoky Mountain Lodge'})
    return render_template('index.html')

# TODO: create REST API endpoint to list available cabins
    ...
    for row in rows:
        ...
        if row[1]:
            occupancy_data.update(
                {row[0].strftime('%m-%d-%Y'): row[1].decode('utf-8')})
